...
GEOMETRY_DEFAULTS = {
    ...
    "cylinder": dict(
        diameter=0.1, length=1.0, U_inf=10.0, Re=100.0
    ),
    "flat_plate": dict(
...
def _build_params(geometry_type, alpha_deg, U_inf, Re, chord, span):
    """Merge UI inputs over geometry defaults."""
    base = dict(GEOMETRY_DEFAULTS.get(geometry_type, {}))
    base["U_inf"] = float(U_inf)
    base["Re"]    = float(Re)
...
def run_pipeline(
    ...
    Always runs CF1 live (pure numpy, ~0.5 s) so the regime map and
    Q-criterion respond to geometry/parameter changes immediately.

    In mock mode the stress predictions are zeroed (no model needed),
    so panels 2–4 are placeholders. Live mode runs full V1 inference.
    """
    try:
        from src.cf1 import solve as cf1_solve

        params = _build_params(geometry_type, alpha_deg, U_inf, Re, chord, span)
        cf1_out = cf1_solve(geometry_type, params, grid_size=8)
        N = cf1_out.grad_u.reshape(-1, 9).shape[0]

        if use_mock:
            # Use CF1 grad_u (real, geometry-specific) but zero stress
            # so the regime map responds to the geometry while the stress
            # panels are clearly placeholders until live inference runs.
            mean_tau     = np.zeros((N, 6), dtype=np.float32).tolist()
            variance_tau = np.zeros((N, 6), dtype=np.float32).tolist()
            status = (
                f"CF1 live ({N} nodes) — stress panels are zero placeholders. "
                "Uncheck 'Quick mode' and click Run for V1 stress predictions."
            )
        else:
            _ensure_model()
            from src.cf2.inference import run_inference
            raw = run_inference(
                graph=cf1_out.graph,
            ...
            status = (
                f"V1 inference complete — {N} nodes, {n_samples} CFM samples"
            )
        ...
        fig = diagnose(result, geometry_type=geometry_type)
        return fig, status
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            use_mock = gr.Checkbox(
                value=True,
                label="Quick mode — CF1 only (regime map live, stress panels zero until unchecked)",
            )
